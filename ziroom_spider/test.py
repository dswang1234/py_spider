...
str_url = "http://sh.ziroom.com/x/792132540.html"
# 获取网页源代码
res = gethtml(str_url)
# 使用lxml的方式解析
...
print('位置')
ziroom_one_location = str(soup.find('span',attrs={"class":"ad"})).split('>')[1].split('<')[0]
print(ziroom_one_location)
# single_info.append(ziroom_one_location)

ziroom_one_found = str(soup.find('span',attrs={"class":"value"})).split('>')[1].split('<')[0]
